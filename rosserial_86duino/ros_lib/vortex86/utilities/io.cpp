...
// lock functions for IO_basePool[...]
    ...
    // TODO: use mutex in IO_POOL_LOCK()
    #define IO_POOL_LOCK()   
    #define IO_POOL_UNLOCK() 
...
DMPAPI(void*) io_Alloc(int io_type, unsigned long io_phyaddr, unsigned long io_size) {
    ...
    switch (base->iotype)
    {
        ...
        case IO_USE_MMIO:
            ...
            #elif defined   DMP_LINUX
                if (IO_mmioFD != -1)
                    base->addr = (unsigned long)mmap(NULL, base->size, PROT_READ | PROT_WRITE, MAP_SHARED, IO_mmioFD, base->addr);
...
DMPAPI(bool) io_Free(void* handle) {
    ...
    switch (base->iotype)
    {
        ...
        case IO_USE_MMIO:
            ...
            #elif defined   DMP_LINUX
                if ((IO_mmioFD != -1) && ((void*)base->addr != (void *)-1)) //&&
                if (munmap((void*)(base->addr), base->size) == -1)
                {
...
// lock functions for PCI_basePool[...]
    ...
    // TODO: use mutex in IO_POOL_LOCK()
    #define PCI_POOL_LOCK()   
    #define PCI_POOL_UNLOCK() 
...
static unsigned long VX86_cpuCLK_DX_A[] = { 0L,    0L,    500L,  600L,  700L,  800L,  900L,  933L,  // for Vortex86DX ver. A/B/C
                                            ...
                                            1433L, 1466L, 1500L };
static unsigned long VX86_cpuCLK_DX_D[] = { 0L,    0L,    500L,  600L,  700L,  800L,  833L,  866L,  // for Vortex86DX ver. D
                                            900L,  933L,  966L,  1000L, 1033L, 1066L, 1100L, 1133L,
                                            1166L, 1200L, 1233L, 1266L, 1300L, 1333L, 1366L, 1400L,
                                            1433L, 1466L, 1500L };
static unsigned long VX86_cpuCLK_MX[] =   { 500L,  600L,  700L,  833L,  866L,  900L,  933L,  966L,  // for Vortex86MX/MX+/DX2
...
static unsigned long get_cpufreq(void) {
    ...
    if (vx86_CpuID() == CPU_VORTEX86DX_D)
    {
        unsigned long strapreg = nb_Read(0x60);
        if ((strapreg & 0x1fL) == 1L) return VX86_dramCLK_DX[(int)((strapreg >> 8) & 0x07L)];
    ...
    if ((vx86_CpuID() == CPU_VORTEX86MX) || (vx86_CpuID() == CPU_VORTEX86MX_PLUS) || (vx86_CpuID() == CPU_VORTEX86DX2))
    {
        unsigned long strapreg = nb_Read(0x60);
        return VX86_cpuCLK_MX[(int)(strapreg & 0x1fL)];
    }
    else
